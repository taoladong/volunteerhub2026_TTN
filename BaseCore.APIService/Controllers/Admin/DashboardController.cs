...
namespace BaseCore.APIService.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        public DashboardController(BaseCore.Repository.EFCore.IUnitOfWork unitOfWork, IEventService eventService)
        {
        ...
        public async Task<IActionResult> GetDashboard()
        {
            ...
            if (role == "Admin")
            {
                ...
                var totalUsers = await _unitOfWork.Users.GetQueryable().CountAsync();
                var pendingEvents = await _unitOfWork.Events.GetQueryable().CountAsync(e => e.Status == "Pending");
                var totalEvents = await _unitOfWork.Events.GetQueryable().CountAsync();
                var totalRegistrations = await _unitOfWork.Registrations.GetQueryable().CountAsync();
                var totalCertificates = await _unitOfWork.Certificates.GetQueryable().CountAsync();
                var pendingKyc = await _unitOfWork.VolunteerProfiles.GetQueryable().CountAsync(p => p.KycStatus == "PendingVerification");
                var pendingOrganizerVerifications = await _unitOfWork.OrganizerVerifications.GetQueryable().CountAsync(v => v.Status == "PendingVerification");
                var pendingSkillVerifications = await _unitOfWork.VolunteerSkills.GetQueryable().CountAsync(vs => vs.VerificationStatus == "PendingVerification");
                var staleDonations = await _unitOfWork.IndividualDonations.GetQueryable().CountAsync(d => d.Status == "PendingConfirmation" && d.CreatedAt <= staleCutoff);
                var failedCertificateJobs = await _unitOfWork.CertificateJobs.GetQueryable().CountAsync(j => j.Status == "Failed");
                var pendingCertificateJobs = await _unitOfWork.CertificateJobs.GetQueryable().CountAsync(j => j.Status == "Pending");
            ...
            {
                var eventIds = await _unitOfWork.Events.GetQueryable()
                    .Where(e => e.OrganizerId == userId)
                    .Select(e => e.Id)
                    .ToListAsync();
                var pendingRegistrations = await _unitOfWork.Registrations.GetQueryable()
                    .CountAsync(r => eventIds.Contains(r.EventId) && r.Status == "Pending");
                var totalVolunteers = await _unitOfWork.Registrations.GetQueryable()
                    .CountAsync(r => eventIds.Contains(r.EventId) && r.Status == "Confirmed");
                var recentEvents = await _unitOfWork.Events.GetQueryable()
                    .Where(e => e.OrganizerId == userId)
                ...
                return Ok(new
                {
                    totalEvents = await _unitOfWork.Events.GetQueryable().CountAsync(e => e.OrganizerId == userId),
                    approvedEvents = await _unitOfWork.Events.GetQueryable().CountAsync(e => e.OrganizerId == userId && e.Status == "Approved"),
                    completedEvents = await _unitOfWork.Events.GetQueryable().CountAsync(e => e.OrganizerId == userId && e.Status == "Completed"),
            ...
            {
                var mySponsors = await _unitOfWork.EventSponsors.GetQueryable()
                    .Include(s => s.Event)
                    .Where(s => s.SponsorId == userId)
                    .OrderByDescending(s => s.SponsoredAt)
                    .ToListAsync();
                var myProposals = await _unitOfWork.SponsorshipProposals.GetQueryable()
            ...
            {
                var myRegistrations = await _unitOfWork.Registrations.GetQueryable()
                    .Include(r => r.Event)
                    .Where(r => r.UserId == userId)
                    .ToListAsync();
                var profile = await _unitOfWork.VolunteerProfiles.GetQueryable().FirstOrDefaultAsync(p => p.UserId == userId);
                var myBadges = await _unitOfWork.UserBadges.GetQueryable()
                    .Include(ub => ub.Badge)
            ...
            [FromQuery] string? status)
        {
            ...
            var baseEvents = await _unitOfWork.Events.GetQueryable()
            ...
            var registrations = eventIds.Count == 0
                ...
                : await _unitOfWork.Registrations.GetQueryable().Where(r => eventIds.Contains(r.EventId)).ToListAsync();

            var certificates = eventIds.Count == 0
                ? new List<BaseCore.Entities.Certificate>()
                : await _unitOfWork.Certificates.GetQueryable().Where(c => eventIds.Contains(c.EventId)).ToListAsync();

            var campaigns = eventIds.Count == 0
                ? new List<BaseCore.Entities.SupportCampaign>()
                : await _unitOfWork.SupportCampaigns.GetQueryable().Where(c => eventIds.Contains(c.EventId)).ToListAsync();
            ...
            var donations = campaignIds.Count == 0
                ...
                : await _unitOfWork.IndividualDonations.GetQueryable().Where(d => campaignIds.Contains(d.CampaignId)).ToListAsync();

            var proposals = eventIds.Count == 0
                ? new List<BaseCore.Entities.SponsorshipProposal>()
                : await _unitOfWork.SponsorshipProposals.GetQueryable().Where(p => eventIds.Contains(p.EventId)).ToListAsync();
