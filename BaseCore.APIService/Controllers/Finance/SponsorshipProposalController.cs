...
namespace BaseCore.APIService.Controllers
{
    ...
    public class SponsorshipProposalController : ControllerBase
    {
        ...
        public async Task<IActionResult> OrganizerRequest(int eventId, [FromBody] OrganizerSponsorshipRequestDto dto)
        {
            ...
            var sponsor = await _unitOfWork.Users.GetQueryable().FirstOrDefaultAsync(u => u.Id == dto.SponsorId && u.UserType == 2 && u.IsActive);
            ...
            await _unitOfWork.SponsorshipProposals.AddAsync(proposal);
        ...
        public async Task<IActionResult> SponsorOffer(int eventId, [FromBody] SponsorSponsorshipOfferDto dto)
        {
            ...
            var sponsor = await _unitOfWork.Users.GetQueryable().AsNoTracking().FirstOrDefaultAsync(u => u.Id == userId);
            var proposal = new SponsorshipProposal
            {
                EventId = eventId,
                SponsorId = userId,
        ...
        public async Task<IActionResult> MarkReceived(int proposalId, [FromBody] ProposalReceivedDto? dto = null)
        {
            ...
            var legacy = proposal.LegacyEventSponsorId.HasValue
                ? await (_unitOfWork.EventSponsors.GetByIdAsync(proposal.LegacyEventSponsorId.Value))
                : null;
            ...
            if (legacy == null)
            {
                ...
                await _unitOfWork.EventSponsors.AddAsync(legacy);
            }
        ...
        {
            return _unitOfWork.SponsorshipProposals.GetQueryable()
                .Include(p => p.Event)
                .Include(p => p.Sponsor)
        ...
        {
            var hasActiveProposal = await _unitOfWork.SponsorshipProposals.GetQueryable().AnyAsync(p =>
                p.EventId == eventId &&
            ...
            var linkedLegacyIds = _unitOfWork.SponsorshipProposals.GetQueryable()
                .Where(p => p.LegacyEventSponsorId != null)
                .Select(p => p.LegacyEventSponsorId!.Value);
            return await _unitOfWork.EventSponsors.GetQueryable().AnyAsync(s =>
                s.EventId == eventId &&
