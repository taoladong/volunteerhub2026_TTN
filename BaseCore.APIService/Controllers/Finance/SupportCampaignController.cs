...
namespace BaseCore.APIService.Controllers
{
    ...
    public class SupportCampaignController : ControllerBase
    {
        ...
        {
            var ev = await _unitOfWork.Events.GetQueryable().AsNoTracking().FirstOrDefaultAsync(e => e.Id == eventId);
            if (ev == null) return NotFound(new { message = "Event not found" });

            var canManage = CanManageEvent(ev, GetUserIdOrNull());
            var query = _unitOfWork.SupportCampaigns.GetQueryable()
                .AsNoTracking()
        ...
        {
            var campaign = await _unitOfWork.SupportCampaigns.GetQueryable()
                .AsNoTracking()
                .Include(c => c.Event)
        ...
        public async Task<IActionResult> Create(int eventId, [FromBody] SupportCampaignDto dto)
        {
            ...
            var ev = await _unitOfWork.Events.GetQueryable().FirstOrDefaultAsync(e => e.Id == eventId);
            if (ev == null) return NotFound(new { message = "Event not found" });
            ...
            await _unitOfWork.SupportCampaigns.AddAsync(campaign);
        ...
        public async Task<IActionResult> Update(int campaignId, [FromBody] SupportCampaignDto dto)
        {
            ...
            var campaign = await _unitOfWork.SupportCampaigns.GetQueryable().Include(c => c.Event).FirstOrDefaultAsync(c => c.Id == campaignId);
            if (campaign == null) return NotFound(new { message = "Campaign not found" });
        ...
        public async Task<IActionResult> ReportCampaign(int campaignId, [FromBody] FinancialReportDto dto)
        {
            ...
            var confirmedAmount = await _unitOfWork.IndividualDonations.GetQueryable()
            ...
            [FromQuery] int pageSize = 50)
        {
            ...
            var query = _unitOfWork.IndividualDonations.GetQueryable()
                .AsNoTracking()
                .Include(d => d.User)
        ...
        public async Task<IActionResult> Donate(int campaignId, [FromBody] IndividualDonationDto dto)
        {
            ...
            await _unitOfWork.IndividualDonations.AddAsync(donation);
            await _unitOfWork.SaveChangesAsync();
        ...
        public async Task<IActionResult> GetMyDonations()
        {
            ...
            var donations = await _unitOfWork.IndividualDonations.GetQueryable()
                .AsNoTracking()
        ...
        public async Task<IActionResult> CancelDonation(int donationId)
        {
            ...
            var donation = await _unitOfWork.IndividualDonations.GetQueryable()
                .Include(d => d.Campaign).ThenInclude(c => c.Event)
        ...
        private async Task<IActionResult> ChangeCampaignStatus(int campaignId, string status)
        {
            ...
            if (status == "Cancelled")
            {
                pendingDonations = await _unitOfWork.IndividualDonations.GetQueryable()
        ...
        {
            return await _unitOfWork.Users.GetQueryable().AnyAsync(u => u.Id == userId && u.IsActive);
        }
        ...
        {
            var donorIds = await _unitOfWork.IndividualDonations.GetQueryable()
                .AsNoTracking()
                .Where(d => d.CampaignId == campaignId && d.Status == "Confirmed")
                .Select(d => d.UserId)
                .Distinct()
