...
namespace BaseCore.APIService.Controllers
{
    ...
    public class RatingsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateRating(int eventId, [FromBody] RatingCreateDto dto)
        {
            ...
            var rater = await (_unitOfWork.Users.GetByIdAsync(userId));
            var ratee = await (_unitOfWork.Users.GetByIdAsync(dto.RateeId));
            if (rater == null || ratee == null) return BadRequest(new { message = "Invalid rating users" });

            var raterAttended = await _unitOfWork.Registrations.GetQueryable().AnyAsync(r =>
                r.EventId == eventId && r.UserId == userId && r.IsAttended);
            var rateeAttended = await _unitOfWork.Registrations.GetQueryable().AnyAsync(r =>
        ...
        public async Task<IActionResult> HideRating(int id, [FromBody] RatingModerationDto? dto)
        {
            if (!int.TryParse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value, out var userId))
                return Unauthorized();
            var rating = await _unitOfWork.Ratings.GetQueryable()
                .Include(r => r.Event)
        ...
        public async Task<IActionResult> DeleteRating(int id)
        {
            ...
            await _unitOfWork.Ratings.DeleteAsync(rating);
            await _unitOfWork.SaveChangesAsync();
