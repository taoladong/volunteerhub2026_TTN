...
namespace BaseCore.Services.VolunteerHub
{
    public class RegistrationService : IRegistrationService
    {
        ...
        public RegistrationService(BaseCore.Repository.EFCore.IUnitOfWork unitOfWork, INotificationService notificationService, IBadgeService badgeService)
        {
        ...
        public async Task<Registration> RegisterAsync(int eventId, int userId, int? shiftId, string? note)
        {
            ...
            if (ev.RequiresKyc)
            {
                var kycStatus = await _unitOfWork.VolunteerProfiles.GetQueryable()
            ...
            if (shiftId.HasValue)
            {
                var shift = await (_unitOfWork.WorkShifts.GetByIdAsync(shiftId.Value))
                    ?? throw new Exception("Shift not found");
                if (shift.EventId != eventId) throw new Exception("Shift does not belong to this event");

                var shiftRegistrations = await _unitOfWork.Registrations.GetQueryable().CountAsync(r =>
                ...
                {
                    var hasSkill = await _unitOfWork.VolunteerSkills.GetQueryable().AnyAsync(vs =>
            ...
            var existing = await _unitOfWork.Registrations.GetQueryable()
            ...
            if (existing != null)
            {
                ...
                var existingVolunteer = await (_unitOfWork.Users.GetByIdAsync(userId));
                await _notificationService.SendAsync(ev.OrganizerId,
                    "Đăng ký lại:", $"{existingVolunteer?.Name} đã đăng ký lại vào sự kiện '{ev.Title}'.",
            ...
            // Notify organizer
            var volunteer = await (_unitOfWork.Users.GetByIdAsync(userId));
            await _notificationService.SendAsync(ev.OrganizerId,
        ...
        {
            var reg = await _unitOfWork.Registrations.GetQueryable()
                .Include(r => r.Event)
                .FirstOrDefaultAsync(r => r.EventId == eventId && r.UserId == userId)
            ...
            var ev = reg.Event ?? await (_unitOfWork.Events.GetByIdAsync(eventId));
        ...
        {
            var reg = await _unitOfWork.Registrations.GetQueryable().Include(r => r.Event)
        ...
        public async Task<Registration> CancelAsync(int eventId, int registrationId, int organizerId)
        {
            ...
            if (wasCancelRequest) throw new Exception("This registration has a pending cancellation request. Please review the request instead of cancelling directly.");
            reg.Status = "Cancelled";
            reg.CancelledAt = DateTime.UtcNow;
            reg.CancelRequested = false;
            var ev = await _unitOfWork.Events.GetByIdAsync(reg.EventId);
            if (ev != null && ev.CurrentParticipants > 0) ev.CurrentParticipants--;
            await _unitOfWork.SaveChangesAsync();
        ...
        public async Task<Registration> CheckInAsync(int eventId, int registrationId, int organizerId, string? qrCode, decimal? latitude = null, decimal? longitude = null)
        {
            ...
            await _notificationService.SendAsync(reg.UserId,
                "User đã check-in",
                $"Bạn đã ghi nhận check in tham gia sự kiện '{reg.Event.Title}'.",
        ...
        {
            var reg = await _unitOfWork.Registrations.GetQueryable().Include(r => r.Event).Include(r => r.Shift)
                .FirstOrDefaultAsync(r => r.EventId == eventId && r.UserId == userId)
                ?? throw new Exception("Registration not found");
        ...
        private static void ValidateCheckInWindow(Registration reg)
        {
            ...
            if (reg.Shift != null)
            {
                if (now < reg.Shift.StartTime.AddMinutes(-15) || now > reg.Shift.EndTime.AddMinutes(30))
                    throw new Exception("Check-in is between (startTime-15mins;endTime+30mins)");
                return;
            }
        ...
        private async Task RefreshVolunteerProgressAsync(int userId, bool evaluateBadges = true)
        {
            ...
            profile.TotalVolunteerHours = await _unitOfWork.Registrations.GetQueryable()
        ...
        {
            var ev = await _unitOfWork.Events.GetQueryable().Include(e => e.WorkShifts).FirstOrDefaultAsync(e => e.Id == eventId)
                ?? throw new Exception("Event not found");
            ...
            var volunteer = await (_unitOfWork.Users.GetByIdAsync(volunteerUserId))
        ...
        {
            return await _unitOfWork.Registrations.GetQueryable()
                .Include(r => r.User)
                .Include(r => r.Shift)
