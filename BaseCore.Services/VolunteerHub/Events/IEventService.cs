...
namespace BaseCore.Services.VolunteerHub
{
    public interface IEventService
    {
        ...
        Task<Dictionary<string, object>> GetImpactAsync(int eventId);
        Task<(List<Dictionary<string, object>> Items, int TotalCount, DateTime Cutoff)> GetOverduePreviewAsync();
        Task<Entities.Event> TransferAsync(int eventId, int newOrganizerId);
        Task<List<Dictionary<string, object>>> GetRegistrationsAsync(int eventId);
        Task<List<Dictionary<string, object>>> GetEventHistoryAsync(int eventId);
    }
}
