...
namespace BaseCore.Services.VolunteerHub
{
    public class ChannelService : IChannelService
    {
        ...
        public ChannelService(
            BaseCore.Repository.EFCore.IUnitOfWork unitOfWork,
        ...
        {
            var channel = await _unitOfWork.Channels.GetQueryable().Include(c => c.Event).FirstOrDefaultAsync(c => c.Id == channelId);
            ...
            var baseQuery = _unitOfWork.Registrations.GetQueryable().Where(r =>
        ...
        {
            return await _unitOfWork.Channels.GetQueryable()
                .Include(c => c.Event)
                .Include(c => c.Shift)
                .Where(c => c.IsActive)
                .ToListAsync();
        ...
        {
            var channel = await _unitOfWork.Channels.GetQueryable()
                .Include(c => c.Event)
                .Include(c => c.Shift)
            ...
            var siblings = await _unitOfWork.Channels.GetQueryable()
                .Include(c => c.Shift)
        ...
        public async Task<Post> CreatePostAsync(int channelId, int authorId, string content, string? imageUrl, string postType, AttachmentDto? attachment)
        {
            ...
            var channel = await _unitOfWork.Channels.GetQueryable().Include(c => c.Event).FirstAsync(c => c.Id == channelId);
            var author = await (_unitOfWork.Users.GetByIdAsync(authorId)) ?? throw new Exception("User not found");
        ...
        {
            var post = await (_unitOfWork.Posts.GetByIdAsync(postId)) ?? throw new Exception("Post not found");
            if (post.ChannelId != channelId) throw new Exception("Post not found in this channel");
        ...
        public async Task<Post> TogglePinAsync(int channelId, int postId, int organizerId, bool isAdmin)
        {
            ...
            if (!post.IsPinned)
            {
                var pinnedCount = await _unitOfWork.Posts.GetQueryable().CountAsync(p => p.ChannelId == channelId && p.IsPinned);
        ...
        {
            var existing = await _unitOfWork.Likes.GetQueryable().FirstOrDefaultAsync(l => l.PostId == postId && l.UserId == userId);
            var post = await (_unitOfWork.Posts.GetByIdAsync(postId)) ?? throw new Exception("Post not found");
            if (post.ChannelId != channelId) throw new Exception("Post not found in this channel");
            ...
            if (existing != null)
            {
                await _unitOfWork.Likes.DeleteAsync(existing);
                post.LikeCount = Math.Max(0, post.LikeCount - 1);
                await _unitOfWork.SaveChangesAsync();
                return false;
            }

            await _unitOfWork.Likes.AddAsync(new Like { PostId = postId, UserId = userId, CreatedAt = DateTime.UtcNow });
            post.LikeCount++;
            await _unitOfWork.SaveChangesAsync();
            return true;
        }
        ...
        {
            return await _unitOfWork.Comments.GetQueryable()
                .Include(c => c.Author)
        ...
        public async Task<Comment> AddCommentAsync(int channelId, int postId, int authorId, string content, int? parentCommentId)
        {
            ...
            var created = await _unitOfWork.Comments.GetQueryable().Include(c => c.Author).FirstAsync(c => c.Id == comment.Id);
        ...
        public async Task<List<object>> GetChannelMembersAsync(int channelId, string? query)
        {
            ...
            var registrations = _unitOfWork.Registrations.GetQueryable().Where(r => r.EventId == channel.EventId && r.Status == "Confirmed");
            if (channel.ShiftId.HasValue)
        ...
        {
            var shift = await _unitOfWork.WorkShifts.GetQueryable().Include(s => s.Event).FirstOrDefaultAsync(s => s.Id == shiftId)
                ?? throw new Exception("Shift not found");
            if (shift.Event.OrganizerId != organizerId) throw new Exception("Not authorized");

            var existing = await _unitOfWork.Channels.GetQueryable().FirstOrDefaultAsync(c => c.ShiftId == shiftId);
            if (existing != null) return existing;

            var parentChannel = await _unitOfWork.Channels.GetQueryable().FirstOrDefaultAsync(c => c.EventId == shift.EventId && c.ParentChannelId == null);
            if (parentChannel == null)
            {
                ...
                await _unitOfWork.Channels.AddAsync(parentChannel);
                await _unitOfWork.SaveChangesAsync();
            }
            ...
            await _unitOfWork.Channels.AddAsync(subChannel);
            await _unitOfWork.SaveChangesAsync();
            return subChannel;
        }
        ...
        public async Task<Poll> CreatePollAsync(int channelId, int postId, int organizerId, bool isAdmin, PollCreateDto dto)
        {
            ...
            var post = await (_unitOfWork.Posts.GetByIdAsync(postId)) ?? throw new Exception("Post not found");
            if (post.ChannelId != channelId) throw new Exception("Post not in this channel");
            if (await _unitOfWork.Polls.GetQueryable().AnyAsync(p => p.PostId == postId)) throw new Exception("Post already has a poll");
        ...
        public async Task<Poll> VoteAsync(int channelId, int pollId, int optionId, int userId)
        {
            ...
            var existingForOption = await _unitOfWork.PollVotes.GetQueryable().AnyAsync(v => v.PollId == pollId && v.OptionId == optionId && v.UserId == userId);
            if (existingForOption) throw new Exception("You already voted for this option");

            if (!poll.AllowMultiple)
            {
                var existing = await _unitOfWork.PollVotes.GetQueryable()
                    .Include(v => v.Option)
            ...
            await _unitOfWork.PollVotes.AddAsync(new PollVote { PollId = pollId, OptionId = optionId, UserId = userId, CreatedAt = DateTime.UtcNow });
            option.VoteCount++;
            await _unitOfWork.SaveChangesAsync();
        ...
        private async Task ProcessMentionsAsync(string content, string entityType, int entityId, int mentionerUserId, int channelId)
        {
            ...
            var users = await _unitOfWork.Users.GetQueryable().Where(u => usernames.Contains(u.UserName)).ToListAsync();
            var mentioner = await (_unitOfWork.Users.GetByIdAsync(mentionerUserId));
            foreach (var user in users)
            {
                if (user.Id == mentionerUserId) continue;
                if (!await CanAccessChannelAsync(channelId, user.Id)) continue;
                var exists = await _unitOfWork.Mentions.GetQueryable().AnyAsync(m =>
                    m.EntityType == entityType &&
                    m.EntityId == entityId &&
                    m.MentionedUserId == user.Id);
                if (exists) continue;

                await _unitOfWork.Mentions.AddAsync(new Mention
                {
